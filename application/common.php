...
include_once "api/library/WeChatPay/WxPay.JsApiPay.php";
include_once "api/library/WeChatPay/WxPay.Config.php";
include_once "api/library/WeChatPay/lib/WxPay.Data.php";
		...
		switch (gettype($var)) {
			...
			case "array":
				...
				foreach ($var as $key => $value) {
					$r[] = "$indent    " . ($indexed ? "" : var_export_short($key) . " => ") . var_export_short($value, "$indent    ");
				}
				return "[\n" . implode(",\n", $r) . "\n" . $indent . "]";
			case "boolean":
...
function weChatPay($orderNo, $money, $openID, $product = "缴纳学费") {
	$tools = new JsApiPay();
	$input = new WxPayUnifiedOrder();
	$input->SetBody($product);
	$input->SetAttach("弘文教育 - " . $product);
	$input->SetOut_trade_no($orderNo);
	$input->SetTotal_fee($money);
	$input->SetTime_start(date("YmdHis"));
	$input->SetTime_expire(date("YmdHis", time() + 600));
	$input->SetGoods_tag($product);
	$input->SetNotify_url("https://xy.gdhwjyedu.com");
...
function CURL($url, $params = null) {
	...
	if (curl_errno($curl)) {
		\think\Log::record(curl_error($curl), 'error');
	}
	curl_close($curl);
	return $output;
}
