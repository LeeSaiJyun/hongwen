...
class PayLog extends Model {
	 ...
	 * @throws \think\exception\DbException
	 */
	public static function updatePayLogPayedAndCommission($data, $isCommission = TRUE) {
		$payLogData = self::get(["order_no" => $data["out_trade_no"], "status" => -1, "money" => $data["total_fee"] * 100]);
		!$payLogData && E("支付记录不存在或已经支付完成");
		$member = User::get(["id" => $payLogData["uid"]]);
		!$member && E("用户不存在");
		self::update(["status" => 1, "return_data" => serialize($data)], ["id" => $payLogData["id"]]);
		if ($isCommission) {
